...
BYTE* Dilation(BYTE* Buffer, int Width, int Height, BYTE* Mask, int m_w, int m_h)
{
	...
	int a, b, tep,stride = 1;//nxn  fxf filter p = f-1/2 n+2p maske gezecek resim buffer yine ayný kalacak
	int padw = (m_w-1)/2;
	int padh = (m_h-1)/2;
	...
	BYTE** newbuf = new BYTE * [h_new];//mask n*n olabilir belli bir degere kadar
	BYTE** Maske = new BYTE * [m_w];
	BYTE** tempbuf = new BYTE * [h_new];
	BYTE* tekbuf = new BYTE[Width * Height];
	for (int i = 0; i < m_h; i++)
	{
		Maske[i] = new BYTE[m_w];
	}
	for (int i = 0; i < h_new; i++)
	{
		newbuf[i] = new BYTE[w_new];
	}
	for (int i = 0; i < h_new; i++)
	{
	...
	//konvolüsyon 
	for (int i = 0; i < h_new - (m_w - 1); i++)
	{
		for (int j = 0; j < w_new - (m_h - 1); j++)
		{
...
BYTE* IncrementContrast(BYTE* Buffer, int N, int M,bool isRGB)
{
	int cur,channel = 3;
	BYTE* newbuf;
	if (isRGB)
	{
		newbuf = new BYTE[N * M * channel];
		for (int i = 0; i < M; i++)
		{
			for (int j = 0; j < N; j++)
			{
				...
				for (int k = 0; k < channel; k++)
				{
					newbuf[channel*cur+k] = Buffer[channel * cur + k]*2;
					if (newbuf[channel * cur + k] > 255)
						newbuf[channel * cur + k] %=255;
				}
			}
		}
	}
...
double* ConvolutionWithPadding(BYTE* Buffer, int Width, int Height, double* Mask, int m_w, int m_h)
{
	int sum = 0, tep,padw,padh;
	padw = (m_w - 1) / 2;
	padh = (m_h - 1) / 2;
	int w_new = Width + 2*padw;//p padding deðeri p = f-1/2 f = mask size
	int h_new = Height + 2*padh;//picture is the same size but buf into the mask n+2*p 


	double** newbuf = new double* [h_new];//mask 3*3 olursaki halidir bu
	double** Maske = new double* [m_w];
	double** tempbuf = new double* [Height];
	double* tekbuf = new double[Height * Width];
	for (int i = 0; i < m_h; i++)
	{
		Maske[i] = new double[m_w];
	}
	for (int i = 0; i < h_new; i++)
	{
		newbuf[i] = new double[w_new];
	}
	for (int i = 0; i < Height; i++)
	{
		tempbuf[i] = new double[Width];
	}
...
//maský oturttur 1 ile carp pixelleri al max ve minini al 2 ye bol merkeze yaz
double* Convolution3DWPSaltPepper(BYTE* Buffer, int Width, int Height ,int m_w, int m_h)
{
	int sum = 0, tep, padw, padh, channel = 3;
	padw = (m_w - 1) / 2;
	padh = (m_h - 1) / 2;
	int w_new = Width + 2 * padw;//p padding deðeri p = f-1/2 f = mask size
	int h_new = Height + 2 * padh;//picture is the same size but buf into the mask n+2*p 
	double* findArr = new double[m_w * m_h];
	...
	int cur,row = 0,col = 0; double minval,maxval;
	for (int d = 0; d < channel; d++)
	{
		for (int i = 0; i < Height; i++)
		{
			for (int j = 0; j < Width; j++)
			{
				...
				col = 0; row = 0;
				maxval = findMax(findArr, m_w * m_h);
				minval = findMin(findArr, m_w * m_h);
				C3Dtempbuf[d][i][j] = (maxval+minval)/2;//both
				//C3Dtempbuf[d][i][j] = maxval;//only pepper
				//C3Dtempbuf[d][i][j] = minval;//only salt
