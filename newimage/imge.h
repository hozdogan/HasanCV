...
BYTE* Digital_ZOOM_RGB_2(BYTE* Buffer, int N, int M,int);

double* Convolution3D(BYTE* Buffer, int Width, int Height, double* Mask, int m_w, int m_h);
double** Digital_ZOOM_d(double** Buffer, int N, int M, int zoom_xk);//bu sadece 2 de calýsýyormus diðerlerince geçiþler cok keskin
BYTE* IncrementContrast(BYTE* Buffer, int N, int M, bool isRGB);
double* ConvolutionWithPadding(BYTE* Buffer, int N, int M, double* Mask, int m_w, int m_h);
double* Convolution3DWithPadding(BYTE* Buffer, int N, int M, double* Mask, int m_w, int m_h);
double* Convolution3DWPSaltPepper(BYTE* Buffer, int N, int M, int m_w, int m_h);
double findMin(double* arr, int size);
double findMax(double* arr, int size);
double* RotationImage(BYTE* Buffer, int Width, int Height, double radius);
